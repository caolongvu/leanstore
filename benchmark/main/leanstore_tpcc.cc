...
auto main(int argc, char **argv) -> int {
  ...
  for (Integer w_id = 1; w_id <= static_cast<Integer>(FLAGS_tpcc_warehouse_count); w_id++) {
    spdlog::debug("Prepare warehouse {}", w_id);
  ...
  db->worker_pool.ScheduleSyncJob(0, [&]() {
    ...
    spdlog::info("Warehouse count: {} - District count: {}", w_cnt, d_cnt);
    ...
    spdlog::info("Customer count: {} - Customer's index count: {}", c_cnt, c_index_cnt);
    assert(c_cnt == c_index_cnt);

    auto o_cnt       = tpcc->order.Count();
    auto o_index_cnt = tpcc->order_wdc.Count();
    spdlog::info("Order count: {} - Order's index count: {}", o_cnt, o_index_cnt);
