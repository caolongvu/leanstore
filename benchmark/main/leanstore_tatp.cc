...
auto main(int argc, char **argv) -> int {
  ...
  for (auto s_id = 1UL; s_id <= FLAGS_tatp_subcriber_count; s_id++) {
    ...
    if (s_id % 100000 == 0) { spdlog::info("Complete %lu subcribers", s_id); }
  }
  ...
  db->worker_pool.ScheduleSyncJob(0, [&]() {
    ...
    spdlog::info("Subcriber count: %lu - Subcriber index count: %lu", s_cnt, s_index_cnt);
    ...
    spdlog::info("Access info count: %lu - Special facility count: %lu", ai_cnt, sf_cnt);
    assert((s_cnt * 2 <= ai_cnt) && (s_cnt * 3 >= ai_cnt));
    assert((s_cnt * 2 <= sf_cnt) && (s_cnt * 3 >= sf_cnt));

    auto cf_cnt = tatp->call_forwarding.Count();
    spdlog::info("Call forwarding count: %lu", cf_cnt);
