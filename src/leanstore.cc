...
void LeanStore::Shutdown() {
  ...
  if (stat_collector.joinable()) {
    ...
    if (FLAGS_txn_debug) {
      ...
      std::ofstream out_latency("/home/long/leanstore/evaluation/tatp/bdr_flush_latencies.csv", std::ios::app);
      ...
      out_throughput << "bdr_flush" << "," << statistics::total_committed_txn.load() / 30.0 << "\n";
    }
  }
}
